...
const updateTotalCategory = async(idCategory, action) => {
    ...
    if (action == "edit") {
        if (idCategory[0] != idCategory[1]) {
            //posicion 0 = valor del select , posicion 1 = valor de la categoria antes de ser editado
            for (let key in idCategory) {
                const category = await getCategory(idCategory[key]).then((res) =>
                    res.data()
                ); //get data para tener el total post
